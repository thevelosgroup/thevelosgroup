...
const PRODUCTS = [
  ...
  { id:'pr2', cat:'printers', sub:'Office & Business', name:'Kyocera Monochrome Laser MFP', price:1650000, img:'printer-kyocera-mono-laser-mfp.jpg', real:true },
  { id:'pr3', cat:'printers', sub:'Commercial & Enterprise', name:'Sharp Commercial Multi-Function Copier', price:8500000, img:'printer-sharp-commercial-copier.jpg', real:true },
  ...
  { id:'pr6', cat:'printers', sub:'Cables', name:'VGA Cable, Standard', price:35000, img:'cable-vga-standard.jpg', real:true },
  { id:'pr7', cat:'printers', sub:'Cables', name:'4K UHD HDMI Cable', price:45000, img:'cable-hdmi-4k-uhd.jpg', real:true },
  { id:'pr8', cat:'printers', sub:'Cables', name:'4K HDMI Cable — Extra-Long (Coiled)', price:85000, img:'cable-hdmi-4k-coiled-long.jpg', real:true },
  { id:'pr9', cat:'printers', sub:'Cables', name:'4K HDMI Cable — Premium Braided', price:65000, img:'cable-hdmi-4k-braided-premium.jpg', real:true },
  ...
  { id:'h1b', cat:'hotel', sub:'Rooms & Lobby', name:'Hotel Room Furniture Package — Walnut Suite', price:5800000, img:'hotel-bedroom-furniture-executive.jpg', real:true },
...
const CATEGORIES = [
  ...
  { key:'tvs', label:'Smart TVs' },
  { key:'audio', label:'Home Theater & Audio' },
  { key:'accessories', label:'Phone Accessories' },
  { key:'cameras', label:'Cameras' },
  { key:'printers', label:'Printers, Copiers & Cables' },
  { key:'appliances', label:'Kitchen & Home Appliances' },
  { key:'furniture', label:'Home Furniture' },
  { key:'hotel', label:'Hotel Furniture & Supply' },
];
...
function productCard(p){
  ...
  return `
    <div class="product-card">
      <div class="img-wrap${p.real ? ' real-tile' : ''}"><img src="${p.img}" alt="${p.name}" loading="lazy" onerror="${fallback}"></div>
...
/* Share a Flash Deal or Ad slide. Tries the native Web Share sheet first —
   on Android/iOS this includes "WhatsApp Status" as a target directly, so
   tapping Share can post it straight to your Status. Falls back to opening
   WhatsApp's own share flow on browsers without native share support
   (mainly desktop). Either way, the shared text carries a WhatsApp order
   link so anyone who taps it lands straight in a chat with dispatch. */
function shareContent(title, bodyText, waLink, pageLink){
    ...
    <div class="spotlight-slide${i===0?' active':''}" data-i="${i}">
      ...
      <div class="spotlight-text">
        ...
        <div class="sl-actions">
          ...
          <button class="btn-share" onclick="shareSpotlight(${i})" aria-label="Share this to WhatsApp Status">${SHARE_ICON}Share</button>
        </div>
      </div>
    </div>`).join('');
...
function renderFlashDeal(){
  ...
  FLASH_DEAL_SHARE = {
    ...
    text: `⚡ FLASH DEAL — ${deal.product.name}\nNow ${formatUGX(deal.discounted)} (was ${formatUGX(deal.product.price)}) — today only!`,
    link
  };
  el.innerHTML = `
    ...
    <div class="fd-text">
      ...
      <div class="fd-actions">
        ...
        <button class="btn-share" onclick="shareFlashDeal()" aria-label="Share this deal to WhatsApp Status">${SHARE_ICON}Share</button>
      </div>
    </div>`;
  startFlashCountdown();
}
...
function shareFlashDeal(){
  if(!FLASH_DEAL_SHARE) return;
  shareContent(FLASH_DEAL_SHARE.title, FLASH_DEAL_SHARE.text, FLASH_DEAL_SHARE.link, siteUrl('shop.html'));
}
