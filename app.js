...
function productCard(p){
  ...
  return `
    <div class="product-card">
      <div class="img-wrap${p.real ? ' real-tile' : ''}"><img src="${p.img}" alt="${p.name}" loading="lazy"></div>
...
function orderNow(id){
  ...
  const msg = `Hello Velos Global Electronics, I would like to order:\n\n${p.name} — ${formatUGX(p.price)}\n\nPlease confirm availability.`;
...
function sendCartOrder(){
  ...
  let lines = ['Hello Velos Global Electronics, I would like to order:', ''];
  let total = 0;
  ids.forEach(id=>{
    const p = PRODUCTS.find(x=>x.id===id);
    const qty = cart[id];
...
const SPOTLIGHT_SLIDES_STATIC = [
  ...
  { tag:'Global Electronics — Smartphones', productId:'ph3', title:null, body:null, media:'light', cta:'View in Store', link:'shop.html' },
  { tag:'Global Electronics — Home Entertainment', productId:'tv7', title:null, body:null, media:'light', cta:'View in Store', link:'shop.html' },
  { tag:'Global Electronics — Laptops', productId:'lp3', title:null, body:null, media:'light', cta:'View in Store', link:'shop.html' },
];
  ...
  track.innerHTML = SPOTLIGHT_SLIDES.map((s,i) => `
    <div class="spotlight-slide${i===0?' active':''}" data-i="${i}">
      ...
      <div class="spotlight-text">
        ...
        <h3>${s.title}</h3>
        <p>${s.body}</p>
        <a class="btn-primary" href="${s.link}" ${s.link.startsWith('http')?'target="_blank"':''}>${s.cta}</a>
      </div>
    </div>`).join('');
...
function renderFlashDeal(){
  ...
  el.innerHTML = `
    <div class="fd-media"><img src="${deal.product.img}" alt="${deal.product.name}" loading="lazy"></div>
    <div class="fd-text">
      <span class="fd-badge">-20% · <span id="flashCountdown"></span></span>
      ...
      <a class="btn-primary" href="${link}" target="_blank">Order This Deal</a>
    </div>`;
  startFlashCountdown();
}
...
function buildSearchIndex(){
  ...
  ['About & Governance','Global Services','Global Electronics','Gift Concierge','Global Designs','Technician Network','Acquisition Policy','Contact'].forEach((label,i) => {
