...
/* Fetches a same-origin image and hands back a File, so it can ride along
   in the native share sheet — this is what makes the photo itself show up
   when someone shares to WhatsApp Status, not just a text link. Only
   works for images actually hosted on this site (same-origin fetch);
   returns null on any failure so callers can fall back to text-only. */
async function imageToShareFile(imgSrc, filename){
  try {
...
/* Share a Flash Deal or Ad slide — WITH its photo attached where the
   browser supports it. Tries the native Web Share sheet first (this is
   what makes "WhatsApp Status" appear as a direct target on Android/iOS,
   photo and caption together, exactly like a Jumia-style promo post).
   Falls back to a WhatsApp forward link (text + URL, no image) on
   browsers without native share or file-sharing support — mainly desktop. */
async function shareContent(title, bodyText, waLink, pageLink, imgSrc, imgName){
  let shareText = bodyText + `\n\nOrder / Enquire on WhatsApp: ${waLink}`;
  if(pageLink) shareText += `\nView more at Velos Global Shop: ${pageLink}`;

  if(navigator.share){
    const file = imgSrc ? await imageToShareFile(imgSrc, imgName || 'velos-deal.jpg') : null;
...
function shareSpotlight(i){
  ...
  if(s.link.startsWith('http')){
    shareContent(s.title, s.text, s.link, null, s.img, 'velos-ad.jpg');
  } else {
    const waLink = `https://wa.me/256755215751?text=${encodeURIComponent('Hello Velos, I am interested in: '+s.title)}`;
    shareContent(s.title, s.text, waLink, siteUrl(s.link), s.img, 'velos-ad.jpg');
  }
}
...
function shareFlashDeal(){
  ...
  shareContent(FLASH_DEAL_SHARE.title, FLASH_DEAL_SHARE.text, FLASH_DEAL_SHARE.link, siteUrl('shop.html'), currentFlashDeal.product.img, 'velos-flash-deal.jpg');
}
...
/* Rotating hero banner for each wing page — purely atmospheric (a short
   ...
   section-head just below). Pages with confirmed real product photography
   get an actual photo rotation; pages without dedicated photography yet
   fall back to a clean brand gradient rather than guessing at filenames
   that might not exist on the server (that's exactly what caused the
   earlier broken-image issue) — give me 2-3 real photos for these wings
   and I'll wire the rotation in the same way. */
const PAGE_HERO_CONFIG = {
  services: { tagline: 'Infrastructure you can trust, dispatched in hours — not days.', images: [] },
  shop: { tagline: 'Real products. Real photos. Real prices — order with confidence.', images: ['phone-samsung-s24ultra.jpg','laptop-lenovo.jpg','camera-sony-a7r-lens-side.jpg','tv-samsung-43.jpg'] },
  gifts: { tagline: "Tell us the moment — we'll find the gift.", images: ['phone-iphone16pro.jpg','camera-sony-a7r-lens-side.jpg'] },
  designs: { tagline: 'Brand, package, and launch — all under one roof.', images: [] },
  technicians: { tagline: 'Verified talent, vetted by Velos HQ.', images: [] },
  about: { tagline: 'Built on radical transparency and zero compromise.', images: [] },
  contact: { tagline: 'One message reaches the whole organization.', images: [] },
};
